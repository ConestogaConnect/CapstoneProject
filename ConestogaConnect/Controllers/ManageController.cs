...
namespace ConestogaConnect.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        [HttpPost]
        public ActionResult UserProfile(UserProfileViewModel model, HttpPostedFileBase ProfileImage)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (ProfileImage!=null && ProfileImage.ContentLength > 1024 * 1024)
                {
                ...
                {
                    var InputFileName = Path.GetFileName(ProfileImage.FileName);
                    var FileExtension = Path.GetExtension(ProfileImage.FileName);
                    
                    if (AllowedFileExtensions.Contains(FileExtension.ToLower()))
                    {
                        var newFileName = User.Identity.GetUserId() + FileExtension;
                        var ServerSavePath = Path.Combine(Server.MapPath("~/UploadImage/") + newFileName);
                        ProfileImage.SaveAs(ServerSavePath);
                        user.ProfileImage = "/UploadImage/" + newFileName;
                    }
                }     
